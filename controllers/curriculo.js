...
exports.cRud_getVersions = async (req, res) => {
    let TokenData = utils.authenticateToken(req);
    if(TokenData === null){
        return res.status(400).send({ status : 400, message: "You are not authorized to perform this action." });
    }
    if(req.params.CienciaId === null){
        return res.status(400).send({ status : 400, message: "Missing parameter CienciaId." });
    }
    curriculo.cRud_getVersions({CienciaId : req.params.CienciaId})
    .then(result => {
        return res.status(200).send(result.recordset);
    })
    .catch(error => {
